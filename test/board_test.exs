defmodule BoardTest do
  ...
  test "is within board" do
    assert within_board?({4,4},{3,3})
  end

  test "is outside board" do
    refute within_board?({4,4},{6,3})
  end

  test "is outside board on boundaries" do
    refute within_board?({4,4},{4,0})
    refute within_board?({4,4},{4,4})
  end

  test "is a neighbour" do
    current = {5,5}
    candidate = {6,6}
    assert alive_neighbour?(current,candidate)
  end
  ...
  test "is not a neighbour" do
    current = {5,5}
    candidate = {6,7}
    refute alive_neighbour?(current,candidate)
  end
  ...
  test "get neighbours" do
    cell = {5,5}
    neighbour_cell = {6,6}
    assert Enum.member?(neighbour_cells(cell), neighbour_cell)
  end

  test "get dead neighbours" do
    board = %GameOfLife.Board{size: {5,5}, alive_cells: %MapSet{}}
    dead_neighbour = [{0,0}]
    assert dead_neighbour == newborn_cells(board,{-1,-1} )
  end
  ...
  test "get newborn cells" do
    ...
    assert MapSet.new([{0,0}]) == newborn_cells(board)
  end
